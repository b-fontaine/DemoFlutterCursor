...
import 'package:demo_flutter_cursor/modules/notifications/providers/models/notification.dart';
...
final localNotifierProvider = Provider<LocalNotifier>((ref) {
  return LocalNotifier(notificationManager: FlutterLocalNotificationsPlugin());
});
...
class LocalNotifier {
  ...
  LocalNotifier({required FlutterLocalNotificationsPlugin notificationManager})
  ...
  Future<void> scheduleDailyAt({
    ...
    const iosDetails = DarwinNotificationDetails(threadIdentifier: kAppName);
  ...
  Future<void> scheduleIn({
    ...
    final scheduledDate = tz.TZDateTime.from(now.add(duration), tz.local);
  ...
  Future<void> scheduleFromNow({
    ...
    final iosDetails = DarwinNotificationDetails(threadIdentifier: channel);
    ...
    await Future.delayed(
      duration,
      () => _notificationManager.show(0, title, body, notificationDetails),
    );
  }
}
